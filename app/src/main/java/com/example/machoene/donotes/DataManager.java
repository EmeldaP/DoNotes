...
/**
 * Created by machoene on 2019-07-23.
 */
...
//            ourInstance.initializeExampleNotes();
    ...
    public static void loadFromDatabase(DoNotesOpenHelper dNHelper){

        SQLiteDatabase db = dNHelper.getReadableDatabase();
        final String[] courseColumns = {
                CourseInfoEntry.COLUMN_COURSE_ID,
                CourseInfoEntry.COLUMN_COURSE_TITLE};

          //cursor maintain position of required results
        final Cursor courseCursor = db.query(CourseInfoEntry.TABLE_NAME, courseColumns,
                null, null, null, null,
                //sorting by courses title
                CourseInfoEntry.COLUMN_COURSE_TITLE
                        //sorting by descending title
        + "DESC");

        loadCoursesFromaDatabase(courseCursor);
    ...
    private static void loadNotesFromDatabase(Cursor cursor) {
        ...
        while (cursor.moveToNext()){
            ...
            //create course to correspont to course id
            CourseInfo noteCourse = dataManager.getCourse(courseId);
            //add note to the courses
    ...
    private static void loadCoursesFromaDatabase(Cursor cursor) {
       int courseIdPos = cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_ID);
       int couseTitlePos = cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_TITLE);

       //ref to singleton
        DataManager dm = getInstance();
        //clear curcirs of previous loading
        dm.mCourses.clear();

        while (cursor.moveToNext()){
            String courseId = cursor.getString(courseIdPos);
            String courseTitle = cursor.getString(couseTitlePos);
